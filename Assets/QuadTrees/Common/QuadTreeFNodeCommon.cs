...
namespace QuadTrees.Common
{
        ...
        where TNode : QuadTreeFNodeCommon<T, TNode, TQuery>
    {
        ...
        public void GetObjects(TQuery searchRect, List<T> list)
        {
            ...
            else if (QueryIntersects(searchRect, Rect))
            {
                ...
                if (ChildTl != null)
                {
                    ...
                    ChildTl.GetObjects(searchRect, list);
                    ChildTr.GetObjects(searchRect, list);
                    ChildBl.GetObjects(searchRect, list);
                    ChildBr.GetObjects(searchRect, list);
                }
                else
                {
        ...
        /// </summary>
        /// <param name="dict">Get all objects in this Quad, and it's children.</param>
        public void GetAllObjects(Dictionary<T, QuadTreeObject<T, TNode>> dict)
        {
            ...
            if (_objects != null)
            {
                ...
                for (int i = 0; i < _objectCount; i++)
                {
                    ...
                    dict.Add(_objects[i].Data, _objects[i]);
                }
            }
            else
            {
            ...
            if (ChildTl != null)
            {
                ChildTl.GetAllObjects(dict);
                ChildTr.GetAllObjects(dict);
                ChildBl.GetAllObjects(dict);
                ChildBr.GetAllObjects(dict);
            }
        }

        /// <summary>
        /// A reference to a list in which to store the objects.
        /// </summary>
        /// <param name="list">Get all objects in this Quad, and it's children.</param>
        public void GetAllObjects(List<T> list)
        {
            ...
            if (ChildTl != null)
            {
                ChildTl.GetAllObjects(list);
                ChildTr.GetAllObjects(list);
                ChildBl.GetAllObjects(list);
                ChildBr.GetAllObjects(list);
            }
        }
